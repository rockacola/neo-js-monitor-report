...
// const START_TIMESTAMP = 1542763597
...
class App {
  constructor() {
    ...
    this.endpointReportModel = this.getEndpointReportModel()
  ...
  getStartMoment() {
    ...
    if (START_TIMESTAMP !== undefined) {
      ...
      const m = moment.unix(START_TIMESTAMP).utc().startOf('minute')
  ...
  getEndpointReportModel() {
    this.logger.debug('getEndpointReportModel triggered.')
  ...
  async performMinuteReport() {
    this.logger.debug('performMinuteReport triggered.')
  ...
  generateReportDocument(endpoint, fromMoment, period, logs) {
    ...
    const highestBlock = _.max(logs, 'blockHeight').blockHeight
  ...
  async setReportDocument(doc) {
    ...
    return new Promise((resolve, reject) => {
      this.endpointReportModel(doc).save((err) => {
        if (err) {
          this.logger.warn('endpointReportModel().save() execution failed.')
          reject(err)
        }
        return resolve()
      })
  ...
  async countReport(endpoint, fromMoment, toMoment) {
    ...
    return new Promise((resolve, reject) => {
      this.endpointReportModel
        ...
        .exec((err, res) => {
          if (err) {
            this.logger.warn('endpointReportModel.count() execution failed. error:', err.message)
            return reject(err)
          }
          return resolve(res)
        })
  ...
  runMinuteReport() {
    this.logger.debug('runMinuteReport triggered.')

    mongoose
      .connect(CONNECTION, { useMongoClient: true })
      .then(() => {
        this.logger.info('database connected.')
        this.performMinuteReport()
      })
      .catch((err) => {
